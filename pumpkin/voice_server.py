...
def _inventory_snapshot(conn) -> Dict[str, Any]:
    ha_summary = store.get_memory(conn, "homeassistant.summary") or {}
...
def _inventory_opportunities(inventory: Dict[str, Any]) -> List[Dict[str, Any]]:
...
class VoiceHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/inventory":
                conn = init_db(str(settings.db_path()), str(settings.repo_root() / "migrations"))
                inventory = _inventory_snapshot(conn)
                opportunities = _inventory_opportunities(inventory)
                _send_json(
                    self,
                    200,
                    {
